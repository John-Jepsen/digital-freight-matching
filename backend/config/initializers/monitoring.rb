...
# Prometheus exporter is provided by yabeda-prometheus; no explicit configuration needed here.
...
class BusinessMetricsCollector
  ...
  # Use configured start time set during boot
  started_at = Rails.application.config.x.started_at rescue nil
  uptime = started_at ? (Time.current - started_at) : 0
...
if defined?(Sidekiq)
  ...
  Sidekiq::Cron::Job.create(
    ...
    class: 'BusinessMetricsCollectionJob'
  )
end
